 ...
 */
function detectLoginForms(options?: { includeAlreadyDetected?: boolean }): LoginForm[] {
...
function handleClickOutside(event: MouseEvent): void {
    if (autofillPopup && !autofillPopup.contains(event.target as Node)) {
        removeAutofillPopup();
    }
}
...
let currentForms: LoginForm[] = [];

/**
 * Fill credentials into the detected form
 */
async function fillCredentials(entryId: string): Promise<void> {
    try {
 ...
 */
function injectVaultIcons(): void {
    currentForms.forEach(({ usernameField, passwordField }) => {
        [usernameField, passwordField].forEach((field) => {
            if (!field || field.hasAttribute('data-vault-icon')) return;
